...
export const SearchResults = ({navigate, location}: SearchResultsProps) => {
	const results = _.range(20).map((i: number) => ({
		gender: "male",
		age: 4,
		username: `test${i}`,
	}))
...
const SearchResult = React.memo(({data}: any) => {
...
const ImgLink = React.memo((
	{showProfile, imgUrl, data, children}:
	{imgUrl: string, showProfile: boolean, data: any, children: JSX.Element[] | JSX.Element}
) => {
	const location = useLocation();
	return <>
		<NavLink to={`/search-results/${data.username}`}
				 state={{from: location.pathname, data: data, imgUrl: imgUrl, scrollTop: document.documentElement.scrollTop}}
				 onClick={e => {if (showProfile) {e.preventDefault()}}}
		>
			{children}
		</NavLink>
	</>
	...
	// return false;
	if (prevProps.imgUrl !== nextProps.imgUrl) {
		console.log("asdflk jasdfimsdlf kasl kjfdl kasdflkj asfdl ksfaldgLink memo:", prevProps.imgUrl === nextProps.imgUrl)
	}
	// return true
	return prevProps.imgUrl === nextProps.imgUrl
})
